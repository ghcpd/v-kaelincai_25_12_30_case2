...
class TestComparisonV1_0_vs_V1_1:
    """
    Side-by-side comparison tests showing v1.0 works but v1.1 fails.
    These tests explicitly demonstrate the regression.
    """
    
    def test_comparison_negative_plus_positive(self):
        """Direct comparison: v1.0 works, v1.1 fails."""
        ...
        # Assert v1.0 is correct
        assert result_v1_0 == -2.0, f"v1.0 should return -2.0, got {result_v1_0}"
        
        # Demonstrate v1.1 regression - it should raise an error or return wrong value
        try:
            result_v1_1 = calc_v1_1.calculate("-5 + 3")
            # If it doesn't crash, check it returns wrong value
            assert result_v1_1 != -2.0, (
                f"REGRESSION: v1.1 returned {result_v1_1}, expected -2.0. "
                f"This shows the bug was introduced in v1.1!"
            )
        except ValueError as e:
            # Bug causes crash - this is also a demonstration of regression
            assert "not enough operands" in str(e), f"v1.1 crashes with: {e}"
    ...
    def test_comparison_double_negative(self):
        ...
        # Demonstrate v1.1 regression
        try:
            result_v1_1 = calc_v1_1.calculate("10 - -5")
            # If it doesn't crash, check it returns wrong value
            assert result_v1_1 != 15.0, (
                f"REGRESSION: v1.1 returned {result_v1_1}, expected 15.0. "
                f"This shows the bug was introduced in v1.1!"
            )
        except ValueError as e:
            # Bug causes crash - this is also a demonstration of regression
